...
class AlphaBetaSymmetryTreeCollector:
    ...
    def __init__(self, ai_symbol: str):
        """
        Initializes the NegaScout tree collector.
    ...
    def build_tree(self, board: Board) -> TreeNode:
        """
        Builds the NegaScout tree from the current board state.
        ...
        self.null_window_searches = 0
        self.re_searches = 0
        self.root = self._build_root(board)
        return self.root

    def _build_root(self, board: Board) -> TreeNode:
        """Builds the root node and its children using NegaScout."""
        node = TreeNode(
            board_state=board.cells.copy(),
        ...
        for i, move in enumerate(available_moves):
            ...
            if i == 0:
                ...
                child = self._build_node(board, 1, -beta, -alpha, -1, move, False, False)
                child.score = -child.score if child.score is not None else 0
            else:
                # Subsequent moves: null-window search first
                self.null_window_searches += 1
                child = self._build_node(board, 1, -alpha - 1, -alpha, -1, move, True, False)
                null_score = -child.score if child.score is not None else 0
                child.null_window_score = null_score

                # If score is within window, re-search with full window
                if alpha < null_score < beta:
                    self.re_searches += 1
                    child = self._build_node(board, 1, -beta, -null_score, -1, move, False, True)
                    child.score = -child.score if child.score is not None else 0
                    child.was_re_searched = True
                    child.null_window_score = null_score
                else:
                    child.score = null_score
    ...
    def _build_node(
        ...
        """
        Recursively builds a NegaScout tree node.
        ...
        Args:
            ...
            alpha: Current alpha value.
            beta: Current beta value.
            color: 1 for AI, -1 for opponent.
            move_made: The move that led to this state.
            is_null_window: Whether this is a null-window search.
            is_re_search: Whether this is a re-search after null-window.

        Returns:
            The constructed TreeNode.
        """
        self.nodes_evaluated += 1
        current_player = self.ai_symbol if color == 1 else self.opponent
        is_maximizing = color == 1
        ...
        node = TreeNode(
            ...
            alpha=alpha,
            beta=beta,
            is_null_window_search=is_null_window,
            was_re_searched=is_re_search
        )
        ...
        if winner:
            node.is_terminal = True
            node.result = 'WIN_X' if winner == PLAYER_X else 'WIN_O'
            raw_score = self._evaluate(board, depth)
            node.score = color * raw_score
            return node
        ...
        available_moves = board.get_available_moves()
        next_player = self.ai_symbol if color == -1 else self.opponent
        ...
        for move in available_moves:
            ...
            if first_child:
                ...
                child = self._build_node(
                    board, depth + 1, -beta, -alpha, -color, move, False, False
                )
                first_child = False
            else:
                # Null-window search
                self.null_window_searches += 1
                child = self._build_node(
                    board, depth + 1, -alpha - 1, -alpha, -color, move, True, False
                )
                child_score = -child.score if child.score is not None else 0
                child.null_window_score = child_score

                # Re-search if necessary
                if alpha < child_score < beta:
                    self.re_searches += 1
                    child = self._build_node(
                        board, depth + 1, -beta, -child_score, -color, move, False, True
                    )
                    child.was_re_searched = True

            board.undo_move(move)

            child_value = -child.score if child.score is not None else 0
